...
const NAV_ITEMS = [
  ...
  { to: '/staff/enrollment-intakes', label: 'Enrollment intakes', icon: UserCheck },
  { to: '/staff/applications', label: 'Assigned requests', icon: ClipboardList },
  { to: '/staff/unassigned', label: 'Unassigned pool', icon: Hand },
  { to: '/staff/queue', label: 'Walk-in queue', icon: Ticket },
  { to: '/staff/appointments', label: 'Appointments', icon: CalendarDays },
  { to: '/staff/workload', label: 'Workload', icon: BarChart3 },
...
export function DashboardLayout({ children, title, subtitle }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F4FAF7]">
      ...
      >
        <header className="sticky top-0 z-30 flex min-h-14 items-center justify-between gap-3 border-b border-[#E2EEE8]/80 bg-white/85 px-4 py-3 shadow-[0_1px_8px_rgba(10,102,64,0.05)] backdrop-blur-md sm:px-6">
          ...
          {(title || subtitle) && (
            <div className="hidden min-w-0 flex-1 lg:block">
              {title ? <p className="truncate text-sm font-bold text-[#052E1C]">{title}</p> : null}
              {subtitle ? <p className="truncate text-xs text-[#4B6358]">{subtitle}</p> : null}
            </div>
          )}
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2 sm:gap-3">
            ...
            <div className="hidden text-right sm:block">
              <p className="text-sm font-semibold leading-none text-[#052E1C]">{user?.name}</p>
              <p className="mt-0.5 text-xs capitalize text-[#6B7280]">{user?.role}</p>
            </div>
            ...
            <div className="hidden h-5 w-px bg-[#E2EEE8] sm:block" />

            <button
              type="button"
              onClick={handleLogout}
              className="flex items-center gap-1.5 rounded-lg border border-[#C4E8D4] bg-white/70 px-2.5 py-1.5 text-xs font-medium text-[#4B6358] transition-all duration-300 hover:border-[#6EE7B7] hover:bg-[#F0FAF5] hover:text-[#0A6640] sm:px-3"
            >
        ...
        <main className="min-h-0 flex-1 overflow-auto bg-[#F4FAF7]">
          <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:py-10">
            {(title || subtitle) && (
              <div className="mb-6 lg:hidden">
                {title ? <p className="text-lg font-bold text-[#052E1C]">{title}</p> : null}
                {subtitle ? <p className="mt-1 text-sm text-[#4B6358]">{subtitle}</p> : null}
              </div>
            )}
            {children}
          </div>
    ...
    <div className={cn('border-t border-[#E2EEE8] p-4', collapsed && 'px-3')}>
      {collapsed ? (
        ...
        <div className="rounded-2xl border border-[#C4E8D4] bg-[#F0FAF5] px-4 py-3">
          ...
          <p className="mt-1 truncate text-sm font-semibold text-[#052E1C]">{instituteName}</p>
        </div>
      )}
    </div>
  );
