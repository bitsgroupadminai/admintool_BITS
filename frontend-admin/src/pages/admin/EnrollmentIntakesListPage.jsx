...
export function EnrollmentIntakesListPage() {
  ...
  const loadIntakes = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await enrollmentIntakesApi.list(params);
  ...
  const columns = [
    ...
    { key: 'offeringName', label: 'Programme' },
    {
      ...
      label: 'Status',
      render: () => (
        <Badge variant="incomplete">Awaiting authorization</Badge>
      ),
    },
    {
      key: 'createdAt',
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
        <div className="mb-8">
          ...
          <p className="mt-2 max-w-3xl text-sm text-[#4B6358]">
            Review new enrollment intakes before applicants can start their application in the student
            portal.
          </p>
        </div>
        ...
        <AdminListBuilder
          ...
          description="Approve authorized applicants or reject and notify them by email."
          ...
          getRowHref={(row) => `/admin/enrollment-intakes/${row.id}`}
          loading={loading}
